...
<div id="content" class="content">
    ...
    <div class="row">
		...
		<button class="btn btn-warning" onclick="add_transaction_manual()"><i class="glyphicon glyphicon-plus"></i> Add Manual</button>
		<br />
		<br />
                ...
                <table class="table table-bordered">
                    ...
                    <form id="form-filter">
                        <tr>
                            ...
                            <td>
                                <button id="btn-filter" type="button" class="btn btn-success">Submit</button>
                            </td>
                        </tr>
                    </form>
	...
	<!-- #nims - modal popup untuk add manual transaction -->
	<div class="modal fade" id="modal-add-manual-transaction" style="display: none;">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="#" id="formTransaction" class="form-horizontal" onsubmit="return false;">
						<!--input type="hidden" value="" name="id"/-->
						<div class="form-group required">
							<label class="col-sm-2 control-label" for="ddTenant">Tenant</label>
							<div class="col-sm-10">
								<input type="hidden" id="ddTenantHdn" name="ddTenantHdn" value="" />
								<select id="ddTenant" name="ddTenant" class="form-control" data-live-search="true" required>
									<option value="">- Select Tenant -</option>
									<?php foreach($info as $rTenant => $valTenant){?>
									<option value="<?php echo $valTenant->id;?>"><?php echo $valTenant->Tenant;?></option>
									<?php } ?>
								</select>
							</div>
						...
						<!--div class="form-group">
							<label class="col-sm-2 control-label" for="txtDevice">Device ID</label>
							<div class="col-sm-10">
								<select id="txtDevice" name="txtDevice" class="form-control" data-live-search="true" required>
						...
						<div class="form-group required">
							<label class="col-md-2 control-label" for="txtTanggal">Tanggal</label>
							<div class="col-md-4">
								<input type="text" id="txtTanggal" class="form-control" name="txtTanggal" value="<?php echo date("Y-m-d");?>" required>
							</div>
							<label class="col-md-2 control-label" for="txtNetSales">Net Sales</label>
							<div class="col-md-4">
								<input type="text" id="txtNetSales" class="form-control" name="txtNetSales" required>
							</div>
						</div>
						<div class="panel-group">
							<div class="panel panel-default">
								...
								<h4 class="panel-title">
									<a class="collapse" data-toggle="collapse" href="#collapse1">MORE</a>
								</h4>
								</div>
								<div id="collapse1" class="panel-collapse collapse">
									<ul class="list-group">
										<li class="list-group-item">
											<div class="form-group">
												 <label class="col-sm-2 control-label" for="txtNoTransaction">NO. Transaction</label>
												<div class="col-md-4">
													<input type="text" id="txtNoTransaction" class="form-control" name="txtNoTransaction"/>
												</div>
												<label class="col-md-2 control-label" for="txtRefSN">Reff. Serial NO</label>
												<div class="col-md-4">
													<input type="text" id="txtRefSN" class="form-control" name="txtRefSN">
												</div>
											</div>
											<div class="form-group">
												<label class="col-md-2 control-label" for="txtAmount">Amount</label>
												<div class="col-md-4">
													<input type="text" id="txtAmount" class="form-control" name="txtAmount">
												</div>
												<label class="col-md-2 control-label" for="txtTax">Tax</label>
												<div class="col-md-4">
													<input type="text" id="txtTax" class="form-control" name="txtTax">
												</div>
											</div>
											<div class="form-group">
												<label class="col-md-2 control-label" for="txtTotal">Total</label>
												<div class="col-md-4">
													<input type="text" id="txtTotal" class="form-control" name="txtTotal">
												</div>
												<label class="col-md-2 control-label" for="txtDiskon">Diskon</label>
												<div class="col-md-4">
													<input type="text" id="txtDiskon" class="form-control" name="txtDiskon">
												</div>  
											</div>
											<div class="form-group">
												<label class="col-md-2 control-label" for="txtKeterangan">Keterangan</label>
												<div class="col-md-10">
													<select id="txtKeterangan" name="txtKeterangan" class="form-control" data-live-search="true">
														<option value=""> </option>
														<option value="Closed">Closed</option>
													</select>
													<!--textarea id="txtKeterangan" class="form-control" name="txtKeterangan"></textarea-->
												</div>
											</div>
										</li>
									</ul>
						...
						<!--div class="form-group">
							<label class="col-md-2 control-label" for="txtJam">Jam</label>
							<div class="col-md-10">
								<div class="input-group bootstrap-timepicker timepicker">
									<input type="text" id="txtJam" class="form-control" name="txtJam">
						...
						<div class="modal-footer">
							<div class="form-group">
								<div class="col-sm-1">
									<button type="submit" id="btnSaveTransaction" name="submit" class="btn btn-primary">Save</button>
								</div>
							</div>
						</div>
					</form>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$("#ddTenant").change(function(){
				var tenantVal = $('select#ddTenant option:selected').val();
				var tenantText = $('select#ddTenant option:selected').text();
				$("#ddTenantHdn").val(tenantText);
				
				$.ajax({
					url : "<?php echo site_url('Transaction/getDeviceByTenanId/')?>"+tenantVal,
					type: "POST",
					dataType: "JSON",
					success: function(data)
					{
						...
						var dd = '<option value="">- Select Devices -</option>';
						for(var i = 0; i < data.length; i++){
							dd += '<option value="'+data[i].DeviceId+'">'+data[i].DeviceId+'</option>';
						}
						$("#txtDevice").append(dd)
					},
					error: function (jqXHR, textStatus, errorThrown)
					{
						alert('Error load device data');
					}
				});
			});
			...
			//function saveTransaction(){
				$('#formTransaction').submit(function(){
					var url;
					if(save_method == 'add') {
						url = "<?php echo site_url('Transaction/addManualData')?>";
					} else {
					...
					$.ajax({
						...
						dataType: "JSON",
						 success: function(data)
						{
							$('#modal-add-manual-transaction').modal('hide');
		...
		function add_transaction_manual(){
			save_method = 'add';
			$('#formTransaction')[0].reset(); // reset form on modals
			$('.form-group').removeClass('has-error'); // clear error class
			$('.help-block').empty(); // clear error string
			$('#modal-add-manual-transaction').modal('show'); // show bootstrap modal
