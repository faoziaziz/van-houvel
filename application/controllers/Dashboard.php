	    ...
	    //$data['map'] = $this->googlemaps->create_map();
			...
			function daily_revenue(){
					...
					/*$query=	$this->db->query("SELECT  sum(a.total1) as total
											...
											");*/
						$query= $this->db->query("SELECT sum(a.dpp) as total from(select a.Nomor, (a.FileTime)as DateTime, a.DeviceId, c.Tenant, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as dpp,if(a.pajak like '%,%',replace(a.pajak,',',''),replace(a.pajak,'.',''))as pajak, if(a.nilaidanpajak like '%,%',replace(a.nilaidanpajak,',',''),replace(a.nilaidanpajak,'.',''))as total from Transaksi a join DeviceTable b on (a.DeviceId = b.DeviceId) join Tenant c on (b.DeviceId = c.DeviceId) WHERE DATE(`filetime`) = CURDATE() ) as a
			...
			function monthly_revenue(){
					...
					$query= $this->db->query("SELECT sum(a.dpp) as total from(select a.Nomor, (a.FileTime)as DateTime, a.DeviceId, c.Tenant, if(a.nilai like '%,%',replace(a.nilai,',',''),replace(a.nilai,'.',''))as dpp,if(a.pajak like '%,%',replace(a.pajak,',',''),replace(a.pajak,'.',''))as pajak, if(a.nilaidanpajak like '%,%',replace(a.nilaidanpajak,',',''),replace(a.nilaidanpajak,'.',''))as total from Transaksi a join DeviceTable b on (a.DeviceId = b.DeviceId) join Tenant c on (b.DeviceId = c.DeviceId) WHERE ((MONTH(a.FileTime)=MONTH(CURDATE())) AND (YEAR(a.FileTime)=YEAR(CURDATE()))) ) as a
